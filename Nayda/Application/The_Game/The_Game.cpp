...
void The_Game::ImplementTreasureArmorAmplifier(std::shared_ptr<CardPlayAllowanceBase> allowance, const GameCardBasis *card, GamerWidget *wt, PositionedCard posCard)
{
    ...
    //Алгоритм:
    //Пусть окончательным решением о применении карты служит подтверждённый выбор
    //конкрентой вещи
    //Если можно использовать, создать новый объект класса CardSelector
    //Результат его использования - возвращение ID карты, к которой будет применён
    //Усилитель
    //Или
